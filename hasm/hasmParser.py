...
#Splits everything into its parts
...
class Parser(object):
    ...
    def _ParseDest(self):
        # D = D +1
        i = self.line.find('=') #finds if '=' exist, else return -1
        if i == -1:
            self.dest = ''      #if '=' is not found, sets dest as empty string
        else:
            self.dest = self.line[:i]   #Else sets dest as everything to the left of '='
