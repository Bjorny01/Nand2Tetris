...
def Pass1(sourceFile):
    ...
    while parser.Advance():
        ...
        if commandType == NO_COMMAND:
            ...
            if symbolTable.Contains(parser.Symbol()) == True:   #Checkar om sybol redan finns i symbolTable
                Error('Symbol already exists ' + parser.Symbol(), parser.LineNo(), parser.Line())
                outFile.write('#Error: Multiple definitions \n')
            elif symbolTable._ValidName(parser.Symbol()) == False:  #Checkar om sybolen namngiven korrekt
                Error('Symbol name is not valid' + parser.Symbol(), parser.LineNo(), parser.Line())
                outFile.write('#Error: Invalid symbol name \n')
            else:
                symbolTable.AddEntry(parser.Symbol(), address)  #Annars lägger till i sybolTable
#################################################################################
# To be completed:
