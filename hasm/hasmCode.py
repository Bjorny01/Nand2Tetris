...
#Translate the "parts" from the Parser and translates them to binary code
...
class Code(object):
    ...
    def Dest(self, mnemonic):
        if len(mnemonic) == 0:
            return '000'    #If nothing specied, stored nowhere
        elif mnemonic in self._destDict:    #If in dictonary returns whats linked to the key
            return self._destDict[mnemonic]
        else:               #If doesnt exist in dictoanry and isnt empty, aka cant be read, return None
            return None
################################################################
# To be completed:
