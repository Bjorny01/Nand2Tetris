...
class CompileEngine(object):
    ...
    def _CompileCall(self, subroutineName=None):
        ...
        Exampel:
        bar(expressionList) | Foo.bar(expressionList) | foo.bar(expressionList)

        Tre fall:
        let p1 = Class.subroutine(expressionList)
        let p1 = var.method(expressionList)
        let p1 = method(expressionList)

            --case--                        --Jack--                     --VM--
        function/constructor        class.subroutine(arguments)         push arg1
                                                                            ...
                                                                        push argn
                                                                        call class.subroutine(n)

        method (operating on var)     var.method(arguemnts)             push var
                                                                        push arg1
                                                                            ...
                                                                        push argn
                                                                        call class.method(n+1)

        method (operating on this)      method(arguments)               push this
                                                                        push arg1
                                                                            ...
                                                                        push argn
                                                                        call class.method(n+1)
        ...
        if sym == '.':                                          #either class.subroutine() or var.method()
            objectName = subroutineName                         #objectName = class|var
            subroutineName = self._ExpectIdentifier()           #subroutineName = function|constructor|method
            self._WriteXml('identifier', self.tokenizer.Identifier())
            self._NextToken()
            ...
            if self.symbolTable.ScopeOf(objectName) == 'subroutine':    #var.method
                variableKind = self.symbolTable.KindOf(objectName)      #ex. local
                index = self.symbolTable.IndexOf(objectName)            #ex. 2
                objectName = self.symbolTable.TypeOf(objectName)        #objectName = class
                nArgs += 1
                self.vmWriter.WritePush(self._KindToSegment(variableKind), index) #push var = push local 2

        else:                                                           #method()
            objectName = self.className
            self._WriteXml('symbol', self.tokenizer.Symbol())
            self.vmWriter.WritePush(SEG_POINTER, 0)                     #push this
            nArgs += 1
        ...
        self.vmWriter.WriteCall(objectName + "." + subroutineName, nArgs)   #Pushes all arguments and calls function class.method|constructor|function
    ...
    def _CompileIf(self):
        ...
        ---VM Code--- AS YOU WANT IT (else present)
            compiled(expression)
            if-goto trueLabel  //if
            goto endIfLabel    //else
        label trueLabel
        ...
        ---VM Code--- AS YOU WANT IT (else not present)
            compiled(expression)
            if-goto trueLabel  //if
            goto endIfLabel
        label trueLabel
        ...
        ---VM Code--- AS I WANT IT
            compiled(expression)
            not
            if-goto trueLabel       //Goes to else if true because of not
            complied(statements1)   //If not true we are on if statement
            goto endIfLabel
        label trueLabel
            compiled(statements2)
        label endIflabel
        ...
        self._CompileStatements()           #Compiled(statements1)

        self._ExpectSymbol('}')
        self._WriteXml('symbol', self.tokenizer.Symbol())
        self._NextToken()

        if self.tokenizer.TokenType() == TK_KEYWORD and self.tokenizer.Keyword() == KW_ELSE:    #If else is present
            self._WriteXml('keyword', self.tokenizer.KeywordStr())                              #Write <keyword> else </keyword>
            self._NextToken()                                                                   #Get next token

            self._ExpectSymbol('{')                                                             #Excpect { and return syntax error if not present
            self._WriteXml('symbol', self.tokenizer.Symbol())                                   #Write <symbol> { </symbol>
            self._NextToken()                                                                   #Get next token

            endLabel = self._UniqueLabel()
            self.vmWriter.WriteGoto(endLabel)  # goto endLabel
            self.vmWriter.WriteLabel(endIfLabel)

            self._CompileStatements()                                                           # Parse statements
        ...
        """
        //This is the best way to do it but TextComparer wont give correct result
        self._WriteXmlTag('<ifStatement>\n')
        ...
        self._CompileExpression()                #vmWrite done in the function - "compiled(expression)"

        self.vmWriter.WriteArithmetic(OP_NOT)    # "not"

        L1 = self._UniqueLabel()
        L2 = self._UniqueLabel()
        ...
        self._CompileStatements()               #vmWrite done in the function - "compiled(statements1)" //Related to the if part, skipped if not not true
        ...
        //If there are no else present, we wont go into if loop so we get
        //         ---VM Code---
        //   compiled(expression)
        //    not
        //    if-goto L1               //Goes to else if true because of not
        //    complied(statements1)   //If not true we are on if statement
        //    goto L2
        //label L1
        //label L2 
        ...
        if self.tokenizer.TokenType() == TK_KEYWORD and self.tokenizer.Keyword() == KW_ELSE:    #If else is present
            ...
            self._CompileStatements()                                                           # Parse statements, vmWrite done in the function
